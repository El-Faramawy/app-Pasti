...
//        $data['image']    = 'uploads/menu/'.$this->saveImage($request->image,'uploads/menu');
        ...
        $this->sendNotification($request['schools'], 'Nuovo Menù disponibile ', 'Nuovo Menù disponibile per '.$days[$day].'  '.date('d-m-Y', strtotime($menu["date"])),'school');
        $this->sendFCMNotification($request['schools'], 'Nuovo Menù disponibile ', 'Nuovo Menù disponibile per '.$days[$day].'  '.date('d-m-Y', strtotime($menu["date"])),'school');
        ...
        if ($request->schools){
            foreach ($request['schools'] as $school){
                ...
                if (!in_array($school,$old_school)){
                    $this->sendNotification([$school], 'Nuovo Menù disponibile ', 'Nuovo Menù disponibile per '.$days[$day].'  '.date('d-m-Y', strtotime($menu["date"])),'school');
                    $this->sendFCMNotification([$school], 'Nuovo Menù disponibile ', 'Nuovo Menù disponibile per '.$days[$day].'  '.date('d-m-Y', strtotime($menu["date"])),'school');
                }
            }
        }
