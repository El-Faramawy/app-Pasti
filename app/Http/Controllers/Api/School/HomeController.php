...
//        return $user_ids;
        ...
        $meal_count = OrderDetails::whereHas('order', function ($query) use ($user_ids) {
            $query->where(['date'=> date('Y-m-d'),['status' , '!=' , 'canceled']])
...
//                    $query->whereBetween('date', [$from_date, $to_date])->whereIn('user_id', $user_ids);
                ...
                foreach ($order_details as $order_detail){
                    if($order_detail->order->user)
                        $users[] = $order_detail->order->user;
                    else
                        $schools[] = School::where('id', school_api()->user()->id)->first();

                }
//                $users[0]==null ? $users = []  : '';
                $meal->users = $users;
                $meal->schools = $schools;
